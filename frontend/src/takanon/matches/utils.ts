...
export function getFullTimeString(fullTime: number[], homeTeam: string, awayTeam: string){
...
export function getExtraTimeString(extraTime: number[], homeTeam: string, awayTeam: string){
...
export function getPenaltiesString(penalties: number[], homeTeam: string, awayTeam: string){
    const [homeGoals, awayGoals] = penalties;
    const winnerTeam = homeGoals > awayGoals ? homeTeam : awayTeam;
    return `${winnerTeam} מנצחת ${homeGoals}-${awayGoals} בפנדלים`;
}

export function getQualifier(fullTime: number[], extraTime?: number[], penalties?: number[]){
