...
export const QuestionBetsLinked = createSelector(
    ...
    (bets, questions, teamsById, playersById) => {
        const betsWithRelations = mapValues(
            ...
            (bet): QuestionBetWithRelations => {
                const relatedQuestion =
                    questions[bet.type_id] ?? ({} as SpecialQuestion)
                const answerType =
                    specialQuestionToAnswerType[relatedQuestion.type]
                return {
                    ...bet,
                    relatedQuestion,
                    answer: getSpecialAnswerModel(
                        bet.answer,
                        answerType,
                        teamsById,
                        playersById
        ...
        return pickBy(
            betsWithRelations,
            (bet) => bet.relatedQuestion && bet.answer
        )
    }
)
