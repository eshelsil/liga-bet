        ...
        {pageVisible && (
            <div>
                <div className={cn("flex items-center justify-between")}>
                    <h1 className='LB-TitleText'>{t('manage.title')}</h1>
                    <InfoOutlined className={cn("w-6 h-6 cursor-pointer fill-white")} onClick={() => openNihusExplanationDialog(undefined)} />
                </div>
                <h4 className='LB-TitleText'>{t('manage.amount', { value: nihusimLeft })}</h4>
                <h4 className='LB-TitleText'>{t('manage.totalGranted', { value: nihusimGranted })}</h4>
                <h4 className='LB-TitleText'>{t('manage.totalUsed', { value: nihusimUsed })}</h4>
                    ...
                    {sortBy(valuesOf(sentNihusim), 'created_at').reverse().map(nihus => (
                        <div key={nihus.id} className={cn("bg-white/80 rounded-xl p-2 text-black mb-5")}>
                            <div className={cn("flex items-center justify-evenly rounded-xl pb-1 gap-4 border-b border-black/50")}>
                                ...
                                <MatchResultV2 {...betToMatchResultProps({...nihus.bet, relatedMatch: nihus.game})} title={t('manage.betTitle')} />
                            </div>
                            ...
                            <div className={cn("flex items-center justify-evenly gap-4 pt-1")}>
                                ...
                                <div className={cn("scale-75")}>
                                    <MatchResultV2
                                        ...
                                        title={isGameLive(nihus.game) ? t('manage.gameLive') : t('manage.gameEnded')}
                                    />
                                </div>
                            </div>
                        </div>
