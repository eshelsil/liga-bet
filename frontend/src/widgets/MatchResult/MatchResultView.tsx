...
function MatchResultView({
    ...
    const hasFullScore =
        typeof home.fullScore === 'number' && typeof away.fullScore === 'number'
    const showFullScore = isQualifierBettable && isTiedGame && hasFullScore
    return (
        <div
            className={`LB-MatchResult ${
                isAutoBet ? 'GameBetsTable-autoBet' : ''
            }`}
        >
            {!!title && <div className="MatchResult-title">{title}</div>}
            <div className="MatchResult-content">
                <div className="MatchResult-side">
                    ...
                    <div className={'MatchResult-row'}>{home.score}</div>
                    {showFullScore && (
                    ...
                    {isQualifierBettable && qualifier === WinnerSide.Home && (
                        <div className={'MatchResult-row'}>✌️</div>
                    )}
                </div>
                    ...
                    {isQualifierBettable && (
                        <>
                            {(!!qualifier || showFullScore) && (
                                <div className={'MatchResult-row'}>90'</div>
                            )}
                            {showFullScore && (
                                <div className={'MatchResult-row'}>120'</div>
                            )}
                            {!!qualifier && (
                                <div className={'MatchResult-row'}>
                ...
                <div className="MatchResult-side">
                    ...
                    <div className={'MatchResult-row'}>{away.score}</div>
                    {showFullScore && (
                        <div className={'MatchResult-row'}>
                            ({away.fullScore})
                        </div>
