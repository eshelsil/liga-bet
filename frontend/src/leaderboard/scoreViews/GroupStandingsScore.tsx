...
function Position({ position, team }: PositionProps) {
    if (!team) {
        return
    }
    const { crest_url, name } = team
    return (
        <div key={position} className="flex-row">
            <span>({position}) </span>
            <TeamWithFlag name={name} crest_url={crest_url}></TeamWithFlag>
        </div>
    )
}
...
function GroupStandingsScore({
    ...
    // }
    return (
        <li key={id} className="list-group-item row flex-row  col-no-padding">
            <div
